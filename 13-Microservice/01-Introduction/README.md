...
- Logic ứng dụng được chia thành các thành phần chi tiết nhỏ với ranh giới trách nhiệm được xác định rõ ràng, phối hợp
  để cùng đưa ra giải pháp.
...
- A dependency injection framework, như là Spring cho phép bạn dễ dàng quản lý project java hơn bằng ngoại hóa(
  externalization cung cấp custom serialization) mối quan hệ giữa các object trong ứng dụng của bạn thông qua quy ước,
...
- Tạo điều kiện tích hợp với Spring Ecosystem (bao gồm Spring Data, Spring Security, Spring Cloud ...).
...
- Để bắt đầu yêu cầu client đầu tiền cần authenticate
  ...
  là cổng vào cho toàn bộ kiến trúc. Nó giao tiếp với Eureka service discovery để lấy ví trí, licensing services và sau
  đó gọi đến các microservice cụ thể.
...
- Một khi request đến `organization service`, nó validate access token, sau khi validated organization service cập nhật
  và lấy những thông tin và gửi về client như một HTTP response. Như một con đường thay thế, một khi thông tin
  organization được cập nhật, organization service thêm một message đến Kafka topic .Licensing service nhận thức được sự
  thay đổi.
- Khi message đến licensing service. [Redis](https://viblo.asia/p/redis-la-gi-LzD5dN2OZjY) lưu thông
  trong `Redis’s in-memory database`. Thông qua quá trình này kiến trúc sử dụng distributed tracing từ Zipkin,
...
- The cloud computing để người dùng chọn mức độ quản lí thông tin và service những cái mà họ cung
  cấp (` anything as a service.`)
    - Infrastructure as a Service (IaaS) : Nhà cung cấp cung cấp cơ sở hạ tầng cho phép bạn truy cập computing resources
      như là servers, storage, và networks.( AWS (EC2), Azure Virtual Machines, Google Compute Engine, và Kubernetes )
    - Container as a Service (CaaS) : Ảo hóa dựa theo vùng chứa ( such as docker )
    - Platform as a Service (PaaS) : Một môi trường tập trung vào development, execution, và maintenance of the
      application (Google App Engine, Cloud Foundry, Heroku, và AWS Elastic Beanstalk)
...
- Right-sized : Cách bạn đảm bảo rằng các dịch vụ nhỏ của bạn có kích thước phù hợp để bạn không phải gánh vác quá nhiều
  trách nhiệm.
- Location transparent : Các bạn quản lý `physical details of service invocation`. Khi trong một microservice
  application, nhiều service instances có thể nhanh trong khởi động và tắt.
...
- Configuration management of service : Làm cách nào để bạn quản lý cấu hình của microservice của mình để nó di chuyển
  giữa environments khác nhau trong the cloud.
...
- The Microservice routing patterns thỏa thuận với cách an client application để cung cấp a microservice discovers the
...
- Fallback pattern: Khi một service gọi thất bại, cho phép service client cố gắng thực hiện công việc của mình thông qua
...
- Nhược điểm của Microservice architecture là khó để debug, trace và monitor những vấn đề bởi vì một hành động đơn giản
...
- The application metrics pattern: monitor metrics và warn các nguyên nhân có thể gây ra lỗi
